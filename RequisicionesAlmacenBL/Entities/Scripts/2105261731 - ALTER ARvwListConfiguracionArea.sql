...
SELECT configuracion.*,
	   ...
	   ISNULL(almacen.Almacenes, 0) AS Almacenes,
	   0 AS EstatusId
FROM ARtblControlMaestroConfiguracionArea AS configuracion
	 ...
	 (
			SELECT ConfiguracionAreaId,
					COUNT(AlmacenId) AS Almacenes
			FROM ARtblControlMaestroConfiguracionAreaAlmacen AS almacen
			WHERE almacen.ConfiguracionAreaId = configuracion.ConfiguracionAreaId
				  AND Borrado = 0
			GROUP BY ConfiguracionAreaId
	 ) AS almacen
